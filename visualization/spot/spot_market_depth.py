...
def prepare_market_depth_figure_to_plot(ticker, param, mark_price, df_with_threshold, threshold, blocks):
    ...
    ax.set_title(
        f"{ticker} Order Book - Depth Chart. Given depth: {threshold}. Depth chosen: {round(param * 100, 2)}%.")
    ax.set_xlabel("Price")
    ax.set_ylabel("USD")
...
def market_depth(ticker, threshold, return_fig=False):
    ...
    df = dp.download_binance_spot_orderbook_depth(client, ticker)

    mark_price = (min((df[df['side'] == 'asks']).price) + max((df[df['side'] == 'bids']).price))/2
